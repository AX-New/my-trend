...
逐个分析东财板块（dc_index），每个板块：搜索新闻 → 构造prompt → 调用LLM。
"""
...
SECTOR_PROMPT = """你是一位专业的板块研究员。根据以下【{name}】板块的最新新闻，完成板块分析。今日：{today}。
...
### 1. 近期概况（recent）
提取**近一个月**的板块重要事件，概括最新动态和短期催化因素。新闻不足时可放宽到三个月并注明。

### 2. 板块分析
- **政策驱动（policy）**：相关产业政策、监管动向、补贴变化
- **市场热度（market_heat）**：板块关注度、资金流入流出、成交活跃度
- **核心标的（core_stocks）**：板块内重要个股的动态、业绩、事件
- **风险提示（risk）**：负面消息、政策转向、估值泡沫等风险
...
- **policy**（政策面，权重25%）：近期政策是否利好本板块
- **market_heat**（市场热度，权重25%）：资金关注度和持续性
- **profitability**（盈利面，权重20%）：板块内公司盈利趋势
- **risk_level**（安全程度，权重15%）：越高=越安全
- **catalyst**（近期催化，权重15%）：近一个月有无重大利好/利空

**score** = policy×0.25 + market_heat×0.25 + profitability×0.20 + risk_level×0.15 + catalyst×0.15

## JSON 输出（严格遵守，不要输出其他内容，总输出不得超过2000个字符）

{{"recent": "近期概况100字", "policy": "政策驱动", "market_heat": "市场热度", "core_stocks": "核心标的", "risk": "风险提示", "sentiment": "乐观|中性|悲观", "summary": "综合评价含关键数据200字", "scores": {{"policy": {{"score": 50, "reason": "理由"}}, "market_heat": {{"score": 50, "reason": "理由"}}, "profitability": {{"score": 50, "reason": "理由"}}, "risk_level": {{"score": 50, "reason": "理由"}}, "catalyst": {{"score": 50, "reason": "理由"}}}}, "score": 50, "key_events": [{{"event": "事件描述含数据200字内", "sentiment": "乐观|悲观|中性", "impact": "影响", "date": "YYYY-MM-DD"}}]}}"""


def analyze_sector(name: str, llm: LLMConfig) -> tuple[dict | None, int]:
    """板块分析，返回 (result, article_count)"""
