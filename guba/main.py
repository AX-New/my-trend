...
用法：
  ...
  python -m guba.main --all-stocks                # 全市场（stock_basic），支持断点续跑
  python -m guba.main --all-la                    # 选股池（la_pick），支持断点续跑
"""
...
from guba.models import GubaSentiment
...
def run_stock(session, llm_cfg, codes: list[str]):
    ...
    logger.info(f"[情绪分析] 帖子抓取完毕，等待 {len(futures)} 个 LLM 任务完成...")
    results = []
    for future in as_completed(futures):
        ...
        for p, label, w in zip(result["posts"], result["labels"], result["weights"]):
        ...
        # _save_post_details(session, code, today, result["posts"], result["labels"], result["weights"])
...
def _load_la_stocks(cfg) -> list[tuple[str, str]]:
    """从 my_stock.la_pick 读取最新日期的选股池，按 code 排序（固定顺序）"""
    import pymysql
    db = cfg.database
    conn = pymysql.connect(host=db.host, port=db.port, user=db.user, password=db.password, db="my_stock")
    cur = conn.cursor()
...
def run_batch(session, llm_cfg, all_stocks: list[tuple[str, str]],
              task_type: str, force_new: bool = False):
    """批量股吧情绪：顺序抓帖子 → 并发 LLM → 逐只入库，游标断点续跑。"""
    if not all_stocks:
        logger.warning(f"[{task_type}] 无股票")
        return

    total = len(all_stocks)
    run = _start_or_resume_batch(session, total, task_type=task_type, force_new=force_new)
    ...
    # 滑动窗口：顺序抓帖子，异步 LLM，逐只推进游标
    pending = {}  # future -> (idx, code, name, timespan)
    ...
    def _flush_completed():
        ...
        for future in done:
            idx, code, name, timespan = pending.pop(future)
            try:
                result = future.result()
            except Exception as e:
                logger.error(f"[{code}] LLM 任务异常: {e}")
            ...
            # _save_post_details(session, code, today,
            #                    result["posts"], result["labels"], result["weights"])
    ...
    for i in range(start_idx, total):
        ...
        if posts:
            ...
            semaphore.acquire()
            future = executor.submit(_llm_task, code, posts, llm_cfg, semaphore)
            pending[future] = (i, code, name, timespan)
        else:
            if (i + 1) % 50 == 0:
                logger.info(f"[{task_type}] 进度 {i+1}/{total}")
    ...
    # 等待剩余 LLM 任务
    logger.info(f"[{task_type}] 帖子抓取完毕，等待 {len(pending)} 个 LLM 任务完成...")
    ...
    elapsed = time.time() - start
    logger.info(
        f"[{task_type}] 完成：{total_saved}/{total} 只有效，"
...
def main():
    ...
    parser.add_argument("--no-resume", action="store_true",
                        help="强制新建 run，不恢复中断的运行（仅 --all-stocks/--all-la）")
    ...
    group.add_argument("--all-stocks", action="store_true",
                       help="全市场股吧情绪（stock_basic），支持断点续跑")
    group.add_argument("--all-la", action="store_true",
                       help="选股池股吧情绪（la_pick），支持断点续跑")
    ...
    try:
        if args.market:
            ...
            logger.info("========== 全市场股吧情绪开始（stock_basic）==========")
            stocks = _load_all_stocks(cfg)
            run_batch(session, cfg.llm, stocks,
                      task_type="guba_all_stocks", force_new=args.no_resume)
            logger.info("========== 全市场股吧情绪完成 ==========")
        elif args.all_la:
            logger.info("========== 选股池股吧情绪开始（la_pick）==========")
            stocks = _load_la_stocks(cfg)
            run_batch(session, cfg.llm, stocks,
                      task_type="guba_all_la", force_new=args.no_resume)
            logger.info("========== 选股池股吧情绪完成 ==========")
        else:
